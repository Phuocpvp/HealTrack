...
export class UserController {
  ...
  async getUserId(@Req() req: Request) {
    const user = req.user as any;  // user is extracted from JwtAuthGuard
    if (!user) {
      return { message: 'User not found' };
    }
    return { userId: user._id };  // Use _id instead of id
  }
  ...
  async getProfileCompletion(@Param('id') id: string) {
    ...
    const completion = await this.userService.calculateProfileCompletion(user);
    return { completion };
  }
